...
public class PortForward {
  ...
  /**
   * Opens the specified port using UPnP. If a port is already open OR is being opened, this method returns false.
   * Note that this has a minimum port of 1 and a maximum port of 65535 -- if it is outside of these bounds, it
   * changes the port to the nearest valid port number (either 1 or 65535).
   *
  ...
  public static void enableUPnP() throws Exception {
    ...
    if (activeGW.getSpecificPortMappingEntry(portToUse, "TCP", portMapping)) {
      ...
      if (activeGW.addPortMapping(portToUse, portToUse, localAddress.getHostAddress(), "TCP", "ShowclixScanner UPnP")) {
        ...
        if (activeGW.deletePortMapping(portToUse, "TCP")) {
          println("Removed port mapping.");
          setGUIPortStatusText("Port " + portToUse + " closed");
        } else {
          println("Unable to remove port mapping :(");
          setGUIPortStatusText("Unable to remove port " + portToUse + " mapping :(");
        }
        threadCreated = false;
